...
export default function NewPostModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const [quotaCurrent, setQuotaCurrent] = useState<number>(50);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs animate-in fade-in">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {type === 'quota' && (
            <div className="p-3 bg-violet-50/50 dark:bg-violet-950/20 border border-violet-200 dark:border-violet-900/40 rounded-xl space-y-2">
              <span className="text-[11px] font-bold text-violet-700 dark:violet-300 uppercase tracking-wider block">
                Quota Progress Metrics
              </span>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[10px] font-semibold text-slate-500 mb-0.5">Current Value</label>
                  <input
                    type="number"
                    value={quotaCurrent}
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-slate-500 mb-0.5">Target Value</label>
                  <input
                    type="number"
                    value={quotaTarget}
          ...
          <div className="pt-2 flex items-center justify-between border-t border-slate-100 dark:border-slate-800">
            ...
            >
              <span>Open full broadcast page</span>
